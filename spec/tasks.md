...
- [ ] **migrate-monorepo-frontend-backend** — 将现有根目录 Next 应用迁入 `frontend/`（含 `src/app`、`public`、前端工具链配置）；将 **Prisma**（`schema.prisma`、`migrations/`、`seed.ts`）及**未来独立 HTTP API** 骨架迁入 `backend/`；根目录 **npm/pnpm workspaces**（或等效 monorepo 编排）与 **CI 工作流路径**、**README 本地命令**一并更新；验收后 `spec/structure.md` 与真实目录一致  
